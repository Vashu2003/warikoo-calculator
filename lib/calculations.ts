...
/* ------------------------------------------------------------------ */
/*  Constants — Warikoo framework thresholds                          */
/* ------------------------------------------------------------------ */

const EQUITY_CAGR = 0.12;
const SAFE_CAGR = 0.08;
const STEPUP_RATE = 0.1;

const EF_MONTHS_TARGET = 6;
const TERM_COVER_INCOME_MULTIPLE = 10;
const HEALTH_COVER_FLOOR = 300_000;
const HEALTH_COVER_WITH_DEPENDENTS = 500_000;

const HIGH_INTEREST_LOAN_THRESHOLD = 9;
const TOXIC_LOAN_THRESHOLD = 18;
...
/**
 * NPER — number of months to pay off `principal` at monthly `payment`
 * given monthly `rate` (decimal, e.g. 0.01).
 * Returns Infinity if payment can't cover monthly interest.
 */
function nper(rate: number, payment: number, principal: number): number {
 ...
 * `monthly` = monthly deposit, `annualRate` decimal, `months` count.
 */
function fvSip(monthly: number, annualRate: number, months: number): number {
...
/**
 * FV of SIP with annual step-up. `stepupRate` decimal (e.g. 0.10).
 */
function fvSipWithStepup(
  monthly: number,
  annualRate: number,
  years: number,
  stepupRate = STEPUP_RATE,
): number {
  ...
  for (let y = 1; y <= years; y++) {
    ...
    total += fvSip(yearlyMonthly, annualRate, 12) * (1 + annualRate) ** (yearsRemaining - 1);
